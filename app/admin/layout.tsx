  ...
  return (
    <div className="absolute gap-4 bg-black z-10 h-full w-full top-0 left-0 p-4 flex">
      ...
      >
        <div className="space-y-4">
          {links.map((linkObj) => {
            return (
              <button
                onClick={() => {
                  setActiveTab(linkObj);
                  router.push(
                    `${
                      linkObj.link != "dashboard"
                        ? `/admin/${linkObj.link}`
                        : "/admin"
                    } `
                  );
        ...
        <div>
          <p className=" text-center text-slate-400 text-sm opacity-50 font-bold">
            v1.3
          </p>
        </div>
      </div>
      <div className="w-4/5 p-8 h-full rounded-md bg-white bg-opacity-5">
        <div className="text-2xl font-bold">{activeTab.title}</div>
        {children}
      </div>
    </div>
  );
